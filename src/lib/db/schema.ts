...
export const attachment_type = pgEnum("attachment_type", [
  "Barrel",
  "Sight",
  "Grip",
  "Underbarrel",
...
export const weapon_type = pgEnum("weapon_type", [
  "AR",
  "HG",
  "LMG",
  "MR",
...
export const loadout = pgTable(
  ...
  (table) => [
    index("idx_operator_id").using(
      "btree",
      table.operator_id.asc().nullsLast().op("int4_ops")
    ),
...
export const operator = pgTable(
  ...
  (table) => [
    index("idx_operator_side").using(
      "btree",
      table.side.asc().nullsLast().op("enum_ops")
    ),
    pgPolicy("Enable read access for all users", {
      as: "permissive",
      for: "select",
...
export const weapon = pgTable(
  ...
  (table) => [
    index("idx_weapon_category").using(
      "btree",
...
export const operator_gadget = pgTable(
  "operator_gadget",
  { operator_id: integer().notNull(), gadget_id: integer().notNull() },
  (table) => [
    index("idx_gadget_id").using(
      "btree",
      table.gadget_id.asc().nullsLast().op("int4_ops")
    ),
    foreignKey({
      columns: [table.operator_id],
      foreignColumns: [operator.id],
    ...
    primaryKey({
      ...
      name: "operator_gadget_operator_id_gadget_id_pk",
    }),
    pgPolicy("Enable read access for all users", {
      as: "permissive",
      for: "select",
...
export const operator_weapon = pgTable(
  "operator_weapon",
  { operator_id: integer().notNull(), weapon_id: integer().notNull() },
  (table) => [
    index("idx_weapon_id").using(
      "btree",
      table.weapon_id.asc().nullsLast().op("int4_ops")
    ),
    foreignKey({
      columns: [table.operator_id],
      foreignColumns: [operator.id],
    ...
    primaryKey({
      ...
      name: "operator_weapon_operator_id_weapon_id_pk",
    }),
    pgPolicy("Enable read access for all users", {
      as: "permissive",
      for: "select",
...
export const operator_weapon_attachment = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.operator_id, table.weapon_id, table.attachment_id],
      name: "operator_weapon_attachment_pkey",
    }),
  ]
);

export const loadout_attachment = pgTable(
  "loadout_attachment",
  {
